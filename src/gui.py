    ...
    def update_canvas(self):
        ...
        for row in self.grid.grid:
            for cell in row:
                ...
                if cell.robot:
                    self.canvas.create_text((x1 + x2) // 2, (y1 + y2) // 2, text=str(cell.robot.id), fill="white")
                if cell.package:
                    self.canvas.create_text((x1 + x2) // 2, (y1 + y2) // 2, text=str(cell.package.id), fill="black")
                if cell.goal:
                    self.canvas.create_text((x1 + x2) // 2, (y1 + y2) // 2, text=str(cell.goal.id), fill="black")
